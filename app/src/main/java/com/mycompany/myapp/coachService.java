...
public class coachService extends Service {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		try{
			...
			if (intent.getIntExtra("CountOfAllIterations",-1) > 0) {Gl_CountOfAllIterations1 = GlnRepeat * intent.getIntExtra("CountOfAllIterations",-1);}
	...
	public String fget_TextToSpeek100(String[] aDealIndexes){
		...
		for (int i=0, len0=aDealIndexes.length; i<len0; i++) {
			...
			while ((Data.aDeals100[Integer.parseInt(   aDealIndexes[i].trim())][   1   ].length > k)
			       && (nCurrProp <= nRandProp)) {//искомый элемент навыка - последний, до которого сумма пропорций < nRandProp  //<= sic!
	...
	public String fget_BeforeTextToSpeek100(String pDealIndexes){
		...
		else  sTextToSpeek += " " + GlParser.SParse( pDealIndexes) + ", "; //-1 sic! 
